...
Supabase should own model profiles, wardrobe states, media assets, approval requests, social drafts, model voice profiles, chat sessions, avatar sessions, subscriptions, agent runs, tool receipts, and analytics snapshots.

## Stack Integration

- Slack: use `#eden-skye-studios` for readiness digests, approval alerts, failures, and receipts after the connected Slack app can see the channel.
- Vercel: previews, API routes, cron triggers, and AI Gateway. Production deploys and env var changes require approval.
- OpenAI / Vercel AI Gateway: Eden chat, content generation, prompt generation, approvals, and Realtime voice architecture. Model/spend changes require approval.
- Supabase: queues, approvals, model records, media, receipts, subscriptions, and analytics. Schema/service-role writes require approval.
- n8n: readiness, trend scan, wardrobe preview, social drafts, approved handoff, analytics, and winner cloning. Workflow activation requires approval.
- Metricool: social draft slots, schedule planning, and analytics. Publishing or live schedule activation requires approval.
- Klaviyo: lifecycle and launch campaign drafts. Sends require approval.
- Shopify: Eden website/store, Black Card products, membership offers, and Xyla source content. Store mutations require approval.
- HeyGen: avatar video and approved live avatar sessions. Live sessions require approval.
...
Allowed: high-fashion lingerie, sheer layered fabric with full coverage, cinematic private-club mood, implied intimacy, platform-safe sensuality.

Blocked: nudity, exposed intimate areas, sexual acts, minors or age ambiguity, pornographic framing, exploitative stereotypes, real-person impersonation.
