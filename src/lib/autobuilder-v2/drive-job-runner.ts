...
export function runDriveJob(request: DriveJobRequest): DriveJobResult {
  ...
  const blocked_operations: Array<Record<string, unknown>> = blockedActionList.map((action) => ({ action, status: "blocked_by_policy", reason: "Action matched blocked_actions policy." }));

  for (const action of acceptedActions) {
    if (action === "create_missing_folders") {
      for (const folder of request.folders ?? []) {
        planned_operations.push({ action, provider: "google_workspace", target: "drive", mode: request.mode ?? "missing_only", dry_run: dryRun, folder, execution_hint: "Use Google Drive files.create with mimeType application/vnd.google-apps.folder." });
      }
    }
    if (action === "upload_file") {
      for (const file of request.files ?? []) {
        planned_operations.push({ action, provider: "google_workspace", target: "drive", dry_run: dryRun, file, execution_hint: "Use Google Drive files.create multipart upload." });
      }
    }
    if (action === "upload_image" || action === "upload_model_images") {
      for (const image of request.images ?? []) {
        planned_operations.push({ action, provider: "google_workspace", target: "drive", dry_run: dryRun, image, execution_hint: "Use Google Drive files.create multipart upload with image MIME type." });
      }
    }
    if (action === "move_file" || action === "move_folder") {
      for (const move of request.moves ?? []) {
        planned_operations.push({ action, provider: "google_workspace", target: "drive", dry_run: dryRun, move, execution_hint: "Use Google Drive files.update with addParents/removeParents." });
      }
    }
    if (action === "list_tree" || action === "validate_tree" || action === "write_receipts") {
      planned_operations.push({
        ...
        execution_hint: action === "list_tree" ? "Use Google Drive files.list scoped to folder parents." : action === "validate_tree" ? "Compare expected folders/files against Drive metadata." : "Write JSON receipt into receipt folder or configured Drive receipts location."
      });
    }
  }
  ...
  return {
    ...
    blocked_operations,
    receipts: [receipt],
    next_actions: [dryRun ? "Re-run with dry_run=false after reviewing planned operations." : "Execute through Drive adapter or n8n bridge.", "Call drive_write_receipt after execution.", "Call drive_list_tree or run_drive_job validate_tree to verify."]
  };
}
