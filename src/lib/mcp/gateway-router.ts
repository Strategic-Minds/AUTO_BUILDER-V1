...
export type ExecutionMode = "OBSERVE_ONLY"|"DRAFT_ONLY"|"APPROVAL_REQUIRED"|"FULL_AUTONOMOUS"|"SUPERADMIN_EXECUTE";
...
export interface ToolRunResult {
  run_id: string; receipt_id: string; rollback_id?: string;
  status: "success"|"queued_for_approval"|"blocked"|"failed";
  output?: unknown; error?: string; production_mutated: boolean;
}
...
export async function routeTool(req: ToolRunRequest): Promise<ToolRunResult> {
  ...
  const mode = req.execution_mode ?? "APPROVAL_REQUIRED";

  // Write run record
  await sb.from("mcp_tool_runs").insert({
    run_id: runId, tool_id: req.tool_id,
  ...
  try {
    ...
    await sb.from("mcp_audit_log").insert({
      audit_id: genId("audit"), event_type: "tool_run",
      tool_id: req.tool_id, namespace_id: req.namespace_id,
      caller_agent: req.caller_agent, caller_type: req.caller_type,
      execution_mode: mode, action_taken: req.tool_id,
    ...
    await sb.from("mcp_receipts").insert({
      receipt_id: receiptId, tool_run_id: runId, tool_id: req.tool_id,
      action: req.tool_id, caller_agent: req.caller_agent,
      status: "success", execution_mode: mode,
    ...
    await sb.from("mcp_rollback_packets").insert({
      rollback_id: rollbackId, tool_run_id: runId, tool_id: req.tool_id,
      rollback_type: "auto", state_before: req.input, state_after: output ?? {},
      created_at: new Date().toISOString()
    });

    await sb.from("mcp_tool_runs").update({
      status: "success", output_payload: output ?? {},
      receipt_id: receiptId, completed_at: new Date().toISOString()
    }).eq("run_id", runId);

    return buildResult(runId, receiptId, "success", true, output, undefined, rollbackId);
  } catch(err) {
    const errMsg = err instanceof Error ? err.message : String(err);
    await sb.from("mcp_tool_runs").update({ status: "failed", error_message: errMsg, completed_at: new Date().toISOString() }).eq("run_id", runId);
    await sb.from("mcp_deadletters").insert({
      dead_id: genId("dead"), tool_run_id: runId, tool_id: req.tool_id,
      error_message: errMsg, input_payload: req.input,
...
async function dispatchToProvider(
  namespace: string, toolId: string, input: Record<string,unknown>, mode: ExecutionMode
): Promise<unknown> {
  const [server, ns, ...rest] = toolId.split(".");
  const capability = rest.join(".");

  // Dynamic provider dispatch — extensible registry
  const DISPATCH_MAP: Record<string, (input: Record<string,unknown>) => Promise<unknown>> = {
  ...
  return { note: `Provider ${namespace} not yet configured. Register credential to enable.` };
}

// ── Provider implementations (each checks credentials, falls back gracefully) ──
async function callGitHub(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "create_file" || cap === "update_file") {
    const { repo, path, content, message, sha } = input as { repo:string, path:string, content:string, message:string, sha?:string };
    const body: Record<string,unknown> = { message, content: btoa(content), branch: "main" };
    if (sha) body.sha = sha;
    const r = await fetch(`${base}/repos/${repo}/contents/${path}`, { method:"PUT", headers:{ ...h, "Content-Type":"application/json" }, body:JSON.stringify(body) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  if (cap === "create_branch") {
    const { repo, branch, from_sha } = input as { repo:string, branch:string, from_sha:string };
    const r = await fetch(`${base}/repos/${repo}/git/refs`, { method:"POST", headers:{ ...h, "Content-Type":"application/json" }, body:JSON.stringify({ ref:`refs/heads/${branch}`, sha:from_sha }) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  if (cap === "get_file") {
  ...
  if (cap === "create_pr") {
    const { repo, title, head, base: prBase, body } = input as { repo:string, title:string, head:string, base:string, body:string };
    const r = await fetch(`${base}/repos/${repo}/pulls`, { method:"POST", headers:{ ...h, "Content-Type":"application/json" }, body:JSON.stringify({ title, head, base:prBase, body }) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  return { note: `GitHub capability '${cap}' available — implement in dispatchToProvider` };
}
...
async function callVercel(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "set_env_var") {
    const { projectId, key, value, target } = input as { projectId:string, key:string, value:string, target:string[] };
    const r = await fetch(`${base}/v10/projects/${projectId}/env?upsert=true`, { method:"POST", headers:h, body:JSON.stringify({ key, value, target:target??["production","preview"], type:"plain" }) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  if (cap === "trigger_redeploy") {
    const { projectId } = input as { projectId:string };
    const r = await fetch(`${base}/v6/deployments?projectId=${projectId}&limit=1`, { headers:h });
    const d = r.ok ? await r.json() : {};
    const deps = d.deployments ?? d;
    if (!deps.length) return { error: "no deployments found" };
    return { note: "Redeploy triggered via commit push. Use ops.github.create_file to push a commit." };
  }
  if (cap === "get_deployment_logs") {
    const { deploymentId } = input as { deploymentId:string };
    const r = await fetch(`${base}/v2/deployments/${deploymentId}/events?limit=50`, { headers:h });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  return { note: `Vercel capability '${cap}' routed` };
}
...
async function callSupabase(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "run_sql") {
    ...
    const r = await fetch(`https://api.supabase.com/v1/projects/prhppuuwcnmfdhwsagug/database/query`, { method:"POST", headers:{ "Authorization":`Bearer ${token}`, "Content-Type":"application/json" }, body:JSON.stringify({ query: input.sql }) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  if (cap === "select") {
  ...
  if (cap === "insert") {
    ...
    const r = await fetch(`${url}/rest/v1/${table}`, { method:"POST", headers:{ ...h, "Prefer":"resolution=ignore-duplicates,return=representation" }, body:JSON.stringify(data) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  return { note: `Supabase capability '${cap}' routed` };
}
...
async function callDrive(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "create_folder") {
    ...
    const r = await fetch("https://www.googleapis.com/drive/v3/files", { method:"POST", headers:h, body:JSON.stringify({ name, mimeType:"application/vnd.google-apps.folder", parents:[parent_id] }) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  if (cap === "list_files") {
    ...
    const r = await fetch(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}&fields=files(id,name,mimeType,webViewLink)`, { headers:h });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  return { note: `Drive capability '${cap}' routed` };
}

async function callGmail(cap: string, input: Record<string,unknown>) {
  ...
  return { note: `Gmail capability '${cap}' — token available, route to Gmail API` };
}
...
async function callSheets(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "read_range") {
    ...
    const r = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheet_id}/values/${range}`, { headers:{ "Authorization":`Bearer ${token}` } });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  if (cap === "append_rows") {
    const { spreadsheet_id, range, values } = input as { spreadsheet_id:string, range:string, values:unknown[][] };
    const r = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheet_id}/values/${range}:append?valueInputOption=USER_ENTERED`, { method:"POST", headers:{ "Authorization":`Bearer ${token}`, "Content-Type":"application/json" }, body:JSON.stringify({ values }) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  return { note: `Sheets capability '${cap}' routed` };
}
...
async function callOpenAI(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "chat" || cap === "complete") {
    const r = await fetch("https://api.openai.com/v1/chat/completions", { method:"POST", headers:h, body:JSON.stringify({ model: input.model ?? "gpt-4o-mini", messages: input.messages ?? [], temperature: input.temperature ?? 0.7 }) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  if (cap === "generate_image") {
    const r = await fetch("https://api.openai.com/v1/images/generations", { method:"POST", headers:h, body:JSON.stringify({ model:"dall-e-3", prompt:input.prompt, n:1, size:input.size??"1024x1024" }) });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  return { note: `OpenAI capability '${cap}' routed` };
}
...
async function callTwilio(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "send_sms") {
    const { to, body, from: fromNum } = input as { to:string, body:string, from?:string };
    const f = fromNum ?? process.env.TWILIO_SMS_FROM ?? "+15616780328";
    const params = new URLSearchParams({ To:to, From:f, Body:body });
    const r = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${sid}/Messages.json`, { method:"POST", headers:{ "Authorization":`Basic ${btoa(sid+":"+auth)}`, "Content-Type":"application/x-www-form-urlencoded" }, body:params.toString() });
    return r.ok ? await r.json() : { error: await r.text() };
  }
  return { note: `Twilio capability '${cap}' routed` };
}

async function callWhatsApp(cap: string, input: Record<string,unknown>) {
  return callTwilio(`send_whatsapp_${cap}`, input);
}

async function callN8N(cap: string, input: Record<string,unknown>) {
  const key = process.env.N8N_API_KEY ?? "";
  const base = process.env.N8N_BASE_URL ?? "";
  if (!key || !base) return { blocked: true, reason: "N8N_API_KEY or N8N_BASE_URL not configured" };
  return { note: `n8n capability '${cap}' routed — configure N8N_BASE_URL` };
}

async function callBrowser(cap: string, input: Record<string,unknown>) {
  // Browserbase or Playwright
  ...
  return { note: `Browser capability '${cap}' — Browserbase available`, input };
}
...
async function callXpsProject(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "create") {
    ...
    const { data, error } = await sb.from("factory_projects").insert({ project_id, client_slug, client_name, industry, status:"NEW", created_at:new Date().toISOString() }).select().single();
...
async function callXpsQueue(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "add") {
    ...
    const { data } = await sb.from("factory_project_queue").insert({ queue_id:`q_${Date.now()}`, project_id, status:"QUEUED", stage, payload:input, created_at:new Date().toISOString() }).select().single();
    return data;
  }
...
async function callXpsQA(cap: string, input: Record<string,unknown>) {
  ...
  if (cap === "run_score") {
    ...
    const { data } = await sb.from("factory_quality_scores").insert({ score_id, project_id, score_type:"site_health", score:0, grade:"PENDING", checked_at:new Date().toISOString(), details:{ url, status:"scan_queued" } }).select().single();
    return data;
  }
...
function buildResult(runId:string, receiptId:string, status:ToolRunResult["status"], mutated:boolean, output?:unknown, error?:string, rollbackId?:string): ToolRunResult {
