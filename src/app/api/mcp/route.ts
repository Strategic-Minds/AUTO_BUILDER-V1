...
function withPlatformAction<T extends Record<string, unknown>>(input: T, action: string) {
...
const handler = createMcpHandler(
  (server) => {
    ...
    server.registerTool('create_vercel_workflow', { title: 'Create Vercel Workflow', description: 'Dry-run-first Vercel workflow or cron planner.', inputSchema: platformProvisioningSchema }, async (input) => mcpText(await runPlatformProvisioningJobTool(withPlatformAction(input as Record<string, unknown>, 'create_vercel_workflow') as never)));
    server.registerTool('create_vercel_agent', { title: 'Create Vercel Agent', description: 'Dry-run-first Vercel agent planner.', inputSchema: platformProvisioningSchema }, async (input) => mcpText(await runPlatformProvisioningJobTool(withPlatformAction(input as Record<string, unknown>, 'create_vercel_agent') as never)));
