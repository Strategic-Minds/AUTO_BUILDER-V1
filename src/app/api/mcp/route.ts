...
const handler = createMcpHandler(
  (server) => {
    ...
    server.registerPrompt("launch_content_machine", { title: "Launch Content Machine", description: "Prompt template for using the MCP as a launch surface for an autonomous content and commerce machine." }, async () => ({ messages: [{ role: "user", content: { type: "text", text: "Use the AUTO BUILDER MCP to inspect the system topology, connector registry, governance policy, capability matrix, Drive job tools, universal job runner, Eden tools, and underscore-safe aliases. Then build the fastest revenue-first content and commerce machine for the current brand, with queue design, approvals, publishing loop, attribution, and a seven-day execution plan." } }] }));
  },
  { instructions: "Use this server as the governed operating surface for AUTO BUILDER. Prefer get_system_topology first, then get_connector_registry, then get_governance_policy, and then route into run_job, run_universal_job, run_eden_job, eden_trend_discovery_dry_run, run_drive_job, drive_create_folder, build_execution_packet, build_content_commerce_machine, or build_universal_integration_blueprint as needed." },
