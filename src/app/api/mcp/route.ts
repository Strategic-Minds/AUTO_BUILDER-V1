...
function buildUniversalIntegrationBlueprint(args: { businessObjective: string; sourceSystems?: string[]; targetSystems?: string[]; trigger?: string }) {
  ...
  return { businessObjective: args.businessObjective, trigger: args.trigger ?? "new asset, new product event, or new campaign brief", sourceSystems, targetSystems, integrationPattern: { orchestrationBrain: "ChatGPT + AUTO BUILDER MCP", stateLayer: "Supabase", eventFlow: ["trigger received", "payload normalized", "routing decision", "queue receipt created", "worker execution", "delivery receipt", "analytics sync", "optimization feedback"], reliability: ["idempotency key", "retry policy", "dead-letter queue", "audit receipt", "operator escalation"] }, connectorPlan: buildConnectorActivationPlan(args.businessObjective, [...sourceSystems, ...targetSystems]), autobuilderV2: { providers: providerRegistry.length, actions: actionCatalog.length } };
}
...
const handler = createMcpHandler(
  (server) => {
    ...
    server.registerTool("get_autobuilder_v2_capabilities", { title: "Get Auto Builder 2 Capabilities", description: "Return all Auto Builder 2 providers and actions exposed by the universal capability bus.", inputSchema: {} }, async () => mcpText({ providers: providerRegistry.length, actions: actionCatalog.length, driveJobTools: ["run_drive_job", "drive_list_tree", "drive_create_folder", "drive_upload_file", "drive_upload_image", "drive_move_file", "drive_move_folder", "drive_write_receipt"], providerRegistry, actionCatalog }));
    ...
    server.registerTool("drive_list_tree", { title: "Drive List Tree", description: "Plan/validate listing a Drive folder tree.", inputSchema: { root_folder_id: z.string() } }, async ({ root_folder_id }) => mcpText(driveListTree(root_folder_id)));
    ...
    server.registerPrompt("launch_content_machine", { title: "Launch Content Machine", description: "Prompt template for using the MCP as a launch surface." }, async () => ({ messages: [{ role: "user", content: { type: "text", text: "Use the AUTO BUILDER MCP to inspect topology, connector registry, governance, v2 capabilities, action catalog, Drive job tools, and validation status. Then build the fastest revenue-first content and commerce machine with queues, receipts, and a seven-day execution plan." } }] }));
  },
  { instructions: "Use this server as the governed operating surface for AUTO BUILDER. Prefer get_system_topology, get_autobuilder_v2_capabilities, get_autobuilder_v2_connectors, validate_autobuilder_v2, then route through execute_autobuilder_v2_action, run_drive_job, or run_autobuilder_v2_workflow as needed." },
