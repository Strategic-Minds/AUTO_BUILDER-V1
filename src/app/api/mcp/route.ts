...
import {
  ...
  driveMoveFileTool,
  driveMoveFolderTool,
  driveInstallZipPackageTool,
  driveUnpackZipToFolderTool,
  driveUploadZipTool,
...
const driveToolNames = ['run_drive_job', 'drive_list_tree', 'drive_create_folder', 'drive_upload_file', 'drive_upload_image', 'drive_move_file', 'drive_move_folder', 'drive_write_receipt', 'drive_upload_zip', 'drive_unpack_zip_to_folder', 'drive_install_zip_package'];
...
const handler = createMcpHandler(
  (server) => {
    ...
    server.registerTool('drive_upload_zip', { title: 'Drive Upload ZIP', description: 'Dry-run or approval-gated raw ZIP upload into allowlisted Google Drive folders.', inputSchema: { source_file_path: z.string(), target_folder_id: z.string(), title: z.string(), mode: z.enum(['dry_run', 'approved_write']).optional(), operator_email: z.string().optional(), receipt_path: z.string().optional() } }, async (payload) => mcpText(await driveUploadZipTool(payload as never)));
    server.registerTool('drive_unpack_zip_to_folder', { title: 'Drive Unpack ZIP To Folder', description: 'Dry-run or approval-gated ZIP unpack into a governed install folder with path traversal and secret-file blocking.', inputSchema: { source_file_path: z.string(), target_folder_id: z.string(), install_folder_name: z.string(), mode: z.enum(['dry_run', 'approved_write']).optional(), operator_email: z.string().optional(), allow_binary_files: z.boolean().optional(), blocked_extensions: z.array(z.string()).optional(), receipt_path: z.string().optional() } }, async (payload) => mcpText(await driveUnpackZipToFolderTool(payload as never)));
    server.registerTool('drive_install_zip_package', { title: 'Drive Install ZIP Package', description: 'Preserve the original ZIP, optionally unpack it, and validate manifest requirements for governed package types.', inputSchema: { source_file_path: z.string(), target_folder_id: z.string(), install_folder_name: z.string(), package_type: z.enum(['eden_shopify_generator_docs', 'eden_visual_source_truth', 'auto_builder_packet', 'general']), mode: z.enum(['dry_run', 'approved_write']).optional(), operator_email: z.string().optional(), unpack: z.boolean().optional(), validate_manifest: z.boolean().optional(), blocked_extensions: z.array(z.string()).optional(), receipt_path: z.string().optional() } }, async (payload) => mcpText(await driveInstallZipPackageTool(payload as never)));
  ...
  { instructions: 'Use this server as the governed operating surface for AUTO BUILDER. Prefer health_check first. New write-capable tools default to dry_run, return receipt and rollback metadata, and must not expose secret values. Route platform provisioning into run_platform_provisioning_job, create_github_repo, create_vercel_project, create_vercel_workflow, create_vercel_sandbox, create_ai_gateway, or create_vercel_agent. Route Drive work into run_drive_job, drive_create_folder, drive_move_file, drive_move_folder, drive_write_receipt, drive_upload_zip, drive_unpack_zip_to_folder, or drive_install_zip_package. Route generic cross-stack work into run_job or run_universal_job. Route rollback planning into rollback. Route browser work into run_browser_job. Route Eden work into run_job, run_eden_job, or eden_trend_discovery_dry_run.' },
