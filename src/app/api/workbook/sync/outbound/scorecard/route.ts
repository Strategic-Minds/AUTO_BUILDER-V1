...
import { loadScorecardWritebackRows, recordSyncRunEnd, recordSyncRunStart, recordWritebackReceipt, writeWorkbookRowsByKey } from "@/lib/workbook/adapters";
...
export async function POST(request: NextRequest) {
  ...
  const sourceKey = assertWorkbookSourceKey(body?.sourceKey ?? "benchmark_integrated");
  const driveFileId = body?.driveFileId ?? WORKBOOK_REGISTRY[sourceKey].driveFileId;
  const sheetName = body?.sheetName ?? "15_Analytics_Scorecard";
  ...
  const runId = await recordSyncRunStart({
    ...
    actor: body?.actor ?? "api/workbook/sync/outbound/scorecard",
  });

  const summary: Record<string, unknown> = { sourceKey, driveFileId, sheetName };

  try {
    const rows = await loadScorecardWritebackRows({ sourceKey });
    ...
    const message = error instanceof Error ? error.message : "Unknown scorecard writeback error";
