...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_mcp-webhook-reconcile_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "mcp-webhook-reconcile", name: "MCP Webhook Reconcile", route: "/api/cron/mcp-webhook-reconcile",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "mcp-webhook-reconcile",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "mcp-webhook-reconcile", server: "mcp_gateway_router", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "mcp-webhook-reconcile", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"mcp-webhook-reconcile", label:"MCP Webhook Reconcile", runId, timestamp: startedAt });
}
