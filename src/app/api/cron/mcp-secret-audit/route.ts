...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_mcp-secret-audit_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "mcp-secret-audit", name: "MCP Secret Audit", route: "/api/cron/mcp-secret-audit",
    server_id: "strategic_minds_universal_ops_mcp", enabled: true,
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "mcp-secret-audit",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "mcp-secret-audit", server: "strategic_minds_universal_ops_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "mcp-secret-audit", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"mcp-secret-audit", label:"MCP Secret Audit", runId, timestamp: startedAt });
}
