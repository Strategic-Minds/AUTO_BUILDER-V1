...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_xps-social-scheduler_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-social-scheduler", name: "XPS Social Scheduler", route: "/api/cron/xps-social-scheduler",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "xps-social-scheduler",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "xps-social-scheduler", server: "xps_website_factory_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-social-scheduler", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"xps-social-scheduler", label:"XPS Social Scheduler", runId, timestamp: startedAt });
}
