...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_xps-calendar-reminders_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-calendar-reminders", name: "XPS Calendar Reminders", route: "/api/cron/xps-calendar-reminders",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "xps-calendar-reminders",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "xps-calendar-reminders", server: "xps_website_factory_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-calendar-reminders", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"xps-calendar-reminders", label:"XPS Calendar Reminders", runId, timestamp: startedAt });
}
