...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_xps-blog-writer_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-blog-writer", name: "XPS Blog Writer", route: "/api/cron/xps-blog-writer",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "xps-blog-writer",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "xps-blog-writer", server: "xps_website_factory_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-blog-writer", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"xps-blog-writer", label:"XPS Blog Writer", runId, timestamp: startedAt });
}
