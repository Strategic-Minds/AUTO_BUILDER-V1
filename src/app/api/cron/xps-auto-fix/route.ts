...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_xps-auto-fix_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-auto-fix", name: "XPS Auto Fix", route: "/api/cron/xps-auto-fix",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "xps-auto-fix",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "xps-auto-fix", server: "xps_website_factory_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-auto-fix", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"xps-auto-fix", label:"XPS Auto Fix", runId, timestamp: startedAt });
}
