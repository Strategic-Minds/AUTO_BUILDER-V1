...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_xps-auto-harden_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-auto-harden", name: "XPS Auto Harden", route: "/api/cron/xps-auto-harden",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "xps-auto-harden",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "xps-auto-harden", server: "xps_website_factory_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-auto-harden", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"xps-auto-harden", label:"XPS Auto Harden", runId, timestamp: startedAt });
}
