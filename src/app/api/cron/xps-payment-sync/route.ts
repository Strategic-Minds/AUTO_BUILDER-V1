...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_xps-payment-sync_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-payment-sync", name: "XPS Payment Sync", route: "/api/cron/xps-payment-sync",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "xps-payment-sync",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "xps-payment-sync", server: "xps_website_factory_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-payment-sync", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"xps-payment-sync", label:"XPS Payment Sync", runId, timestamp: startedAt });
}
