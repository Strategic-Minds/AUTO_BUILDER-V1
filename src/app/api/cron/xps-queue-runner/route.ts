...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_xps-queue-runner_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-queue-runner", name: "XPS Queue Runner", route: "/api/cron/xps-queue-runner",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "xps-queue-runner",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "xps-queue-runner", server: "xps_website_factory_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-queue-runner", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"xps-queue-runner", label:"XPS Queue Runner", runId, timestamp: startedAt });
}
