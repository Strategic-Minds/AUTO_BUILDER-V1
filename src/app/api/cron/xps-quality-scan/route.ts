...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_xps-quality-scan_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-quality-scan", name: "XPS Quality Scan", route: "/api/cron/xps-quality-scan",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "xps-quality-scan",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "xps-quality-scan", server: "xps_website_factory_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-quality-scan", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"xps-quality-scan", label:"XPS Quality Scan", runId, timestamp: startedAt });
}
