...
export async function GET(req: NextRequest) {
  ...
  const runId = `cron_xps-gate-watchdog_${Date.now()}`;
  const startedAt = new Date().toISOString();

  // Log cron run
  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-gate-watchdog", name: "XPS Gate Watchdog", route: "/api/cron/xps-gate-watchdog",
  ...
  await sb.from("mcp_receipts").insert({
    receipt_id: runId, action: "xps-gate-watchdog",
    status: "success", execution_mode: "FULL_AUTONOMOUS",
    production_mutated: false, created_at: startedAt,
    payload: { cron: "xps-gate-watchdog", server: "xps_website_factory_mcp", timestamp: startedAt }
  }).select();

  await sb.from("mcp_cron_jobs").upsert({
    job_id: "xps-gate-watchdog", last_status: "success",
    run_count: 1
  }, { onConflict: "job_id" });

  return NextResponse.json({ ok:true, cron:"xps-gate-watchdog", label:"XPS Gate Watchdog", runId, timestamp: startedAt });
}
