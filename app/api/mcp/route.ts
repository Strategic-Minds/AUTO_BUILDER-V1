...
import { ALL_TOOLS, MCP_VERSION, GATEWAY_VERSION, createMCPResponse, createMCPError, createReceiptId } from '@/lib/mcp/server';
...
async function executeTool(name: string, args: Record<string, unknown>, receiptId: string): Promise<unknown> {
  ...
  switch (name) {
    ...
    case 'system.audit_trail':
      return { message: 'Audit trail requires Base44 entity connection', receipt_id: receiptId, project_id: args.project_id ?? 'all' };

    case 'system.cost_status':
      return { status: 'within_budget', message: 'Cost tracking via Supabase cost_events table', receipt_id: receiptId };
    ...
    case 'base44.scoring_summary': {
      return { message: 'Connect to Base44 ScoringRegistry entity to read ceiling scores', app_id: BASE44_APP_ID, receipt_id: receiptId };
    }
    ...
    case 'github.list_repos': {
      ...
      return { repos: (repos as any[]).map((r: any) => ({ name: r.name, url: r.html_url, default_branch: r.default_branch })), receipt_id: receiptId };
    }
    ...
    case 'github.list_prs': {
      ...
      return { prs: (prs as any[]).map((p: any) => ({ number: p.number, title: p.title, state: p.state, draft: p.draft, url: p.html_url })), receipt_id: receiptId };
    }

    case 'intel.scoring_dashboard':
      return { message: 'REALITY OS Ceiling Score', target: '95+/110', dimensions: ['build_integrity/20', 'code_quality/20', 'self_healing/15', 'test_coverage/20', 'receipt_integrity/10', 'ai_capability/15', 'security_posture/10'], note: 'Connect Base44 ScoringRegistry for live scores', receipt_id: receiptId };

    case 'intel.self_reflection_report':
      return { message: 'Latest self-reflection cycle data', source: 'Base44 ValidationRegistry', app_id: BASE44_APP_ID, receipt_id: receiptId };

    case 'intel.repair_queue':
      return { message: 'Repair queue data', source: 'Base44 RepairQueue entity', app_id: BASE44_APP_ID, receipt_id: receiptId };

    case 'intel.source_truth_audit':
      return { message: 'Source truth audit initiated', scope: args.scope ?? 'full', receipt_id: receiptId, note: 'Full audit requires live connector access' };
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(createMCPError(0, -32700, 'Parse error', e.message), { status: 400 });
  }
}
